...
int main(void) {
    int i, j, interest=1, duration, n_deprs, curr_month, 
        depr_month[MAX_DEPR];
    double value, downpayment, repayment, depr_perc[MAX_DEPR];
    Loan carLoan;
    ...
    while(cin >> duration >> downpayment >> value >> n_deprs) {
        ...
        for (i=0; i<n_deprs; i++) {
            cin >> depr_month[i] >> depr_perc[i];
        }
        
        curr_month=0; 
        //initial depreciation
        carLoan.updateDepreciation(depr_perc[0], 1);
        
        
        
        
        /* using next_depr_month
        while(!carLoan.loanLessThanCar()) {
            curr_month++;
    ...
    */
        ...
        while(curr_month <= MAX_DUR) {
            cout << curr_month << " " <<curr_depr_month << " " <<next_depr_month << " " << n_deprs << endl;
            if (curr_month == next_depr_month) {
                //update depreciation month, percentage and month of next depr rate
                curr_depr_month = next_depr_month;
                cin >> depr_perc;
    ...
    */
    ...
    for (i=1; i<n_deprs; i++) {
        cin >> curr_depr_month >> depr_perc;
        cin >> next_depr_month;
        for (curr_month >= curr_depr_month) {
